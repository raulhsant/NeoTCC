...
Nessa seção são apresentados so circuitos dos sistemas compostos pela conexão dos estágios projetados até aqui. Os circuitos serão expostos nas figuras~\ref{fig:sys_cuk_conv_bip} a ~\ref{fig:sys_cuk_int_uni} e no anexo~\ref{anex:circ_images}, com maior qualidade.
% \subsubsection{Conversor Ćuk Convencional com Inversor VSI Bipolar}
% \begin{figure}[htb]%
% 	\begin{center}%
% 		\includegraphics[width=0.55 \linewidth]{lcl_filter_psim}
% 		\caption{Filtro LCL implementado}
% 		\label{fig:sys_cuk_conv_bip}
% 	\end{center}
% \end{figure}
