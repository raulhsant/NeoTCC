...
Podem ser empregados três diferentes tipos de filtros L, LC e LCL. Destes, o último é mais utilizado atualmente devido a sua maior eficiência\cite{LCL_FILTER}.
...
% Os componentes do filtro são encontrados a partir das equações~\ref{eq:f_res_eq1} a \ref{eq:lcl_deltaI2}. Para os cálculos serão utilizados $f = 60Hz$, $V = 180V$ e $P = 300W$. Os limites da potência reativa gerada pelo capacitor e da variação de tensão serão $5\%$ e $10\%$, respectivamente.

% Dessa forma, têm-se os valores apresentados nas equações~\ref{fig:LCL_comp1} a \ref{fig:LCL_compX}.
